...
If you want to install a specific version of Deep Security Smart Check, you can use the archive link for the tagged release rather than for `master`. For example, to install Deep Security Smart Check 1.1.4, you can run:
...
helm install \
  ...
  --set activationCode={activation code} \
  --name deepsecurity-smartcheck \
  https://github.com/deep-security/smartcheck-helm/archive/1.1.4.tar.gz
```
...
| `networkPolicy.enabled`        | `true`                                        | **EXPERIMENTAL:** Whether Kubernetes `NetworkPolicy` resources should be created for the deployed pods.                                                                                                                                                                     |
